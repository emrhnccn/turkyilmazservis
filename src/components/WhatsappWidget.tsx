...
export default function WhatsappWidget({ phone = '0552 116 41 28', isStatic = false }: WhatsAppWidgetProps) {
  ...
  const CardContent = (
    <div className="w-full bg-zinc-900 rounded-3xl shadow-2xl border border-zinc-800 overflow-hidden text-left">
      {/* Kart Başlığı (Kırmızı - Rose Gradient) */}
      <div className="bg-gradient-to-r from-red-600 via-rose-600 to-red-600 p-4 text-white relative">
        {!isStatic && (
          <button
        ...
        <div className="flex items-center gap-3">
          <div className="relative">
            <div className="w-12 h-12 bg-zinc-950 rounded-full flex items-center justify-center text-red-500 shadow-md border border-white/10">
              <MessageCircle className="w-7 h-7 fill-current" />
            </div>
            <span className="absolute bottom-0 right-0 w-3.5 h-3.5 bg-red-500 border-2 border-zinc-900 rounded-full animate-pulse"></span>
          </div>
          <div>
      ...
      <div className="p-4 bg-zinc-950 space-y-3">
          ...
          <div className="flex flex-wrap gap-1.5">
            {quickTemplates.map((t, idx) => (
              <button
                ...
                className="text-left text-xs bg-zinc-900 hover:bg-red-950/50 hover:text-red-400 hover:border-red-600/50 border border-zinc-800 text-zinc-300 px-3 py-1.5 rounded-xl transition font-medium cursor-pointer"
              >
                {t.title}
              </button>
            ))}
        ...
        {/* Mesaj Yazma Alanı */}
        <div className="pt-2">
          <div className="flex items-center gap-2 bg-zinc-900 border border-zinc-800 rounded-2xl p-1.5 pl-3 focus-within:border-red-600 focus-within:ring-1 focus-within:ring-red-600/20 transition">
            <input
              type="text"
            ...
            <button
              ...
              className="w-9 h-9 rounded-xl bg-gradient-to-r from-red-600 to-rose-600 hover:from-red-700 disabled:opacity-30 text-white flex items-center justify-center shrink-0 transition shadow-sm cursor-pointer"
            >
  ...
  return (
    <>
      {/* MASAÜSTÜ İÇİN KIRMIZI WHATSAPP BUTONU VE POPUP TOOLTIP */}
      <div className="fixed bottom-6 right-6 z-50 flex flex-col items-end">
        ...
        {!isOpen && showTooltip && (
          <div className="mb-3 max-w-[260px] sm:max-w-[290px] bg-zinc-900/95 backdrop-blur-md border border-red-600/40 text-white p-3.5 rounded-2xl rounded-br-xs shadow-2xl shadow-red-600/15 relative animate-bounce duration-1000">
            <button
              onClick={(e) => {
                e.stopPropagation()
            ...
            >
              <div className="flex items-center gap-1.5 text-red-500 font-extrabold text-[11px] mb-1 uppercase tracking-wider">
            ...
            <div className="absolute -bottom-1.5 right-6 w-3 h-3 bg-zinc-900 border-r border-b border-red-600/40 transform rotate-45" />
          </div>
        )}
        ...
        <div className="flex items-center gap-3">
          <button
            ...
            className="relative group w-16 h-16 bg-gradient-to-tr from-red-600 via-rose-600 to-red-600 hover:from-red-500 hover:to-rose-500 text-white rounded-full flex items-center justify-center shadow-2xl shadow-red-600/50 hover:scale-110 active:scale-95 transition-all duration-300 cursor-pointer"
          >
            {isOpen ? (
              ...
              <>
                ...
                <span className="absolute -inset-2 rounded-full bg-red-600 opacity-40 animate-ping -z-10" />
                
                <span className="absolute -top-1 -right-1 flex h-4 w-4">
                  <span className="relative inline-flex rounded-full h-4 w-4 bg-zinc-950 border-2 border-red-600 font-black text-[9px] text-red-500 items-center justify-center">
                    1
                  </span>
                </span>
              </>
      ...
      <div className="fixed bottom-0 left-0 right-0 z-40 bg-zinc-950/95 backdrop-blur-xl border-t border-zinc-800 p-2.5 sm:hidden flex items-center gap-2">
        ...
        <button
          ...
          className="flex-1 inline-flex items-center justify-center gap-2 bg-gradient-to-r from-red-600 via-rose-600 to-red-700 text-white font-extrabold text-xs py-3 rounded-xl shadow-lg shadow-red-600/25 active:scale-95 transition cursor-pointer"
        >
